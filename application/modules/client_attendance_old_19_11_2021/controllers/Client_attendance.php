...
class Client_attendance extends MY_Controller {
	...
	public function client_emp_details(){
        ...
        echo json_encode(array('client_name' => $client->client_name, 'emp_code' => $emp->employee_code, 'emp_name' => $emp->employee_name, 'father_name' => $emp->father_name,'branch_name' => $branch->branch_name, 'desig_name' => $designation->desig_name, 'bill_calculation_days' => $billing_costing->bill_calculation_days, 'duty_hrs' => $billing_costing->duty_hrs,'cnt_advance' => $advance->cnt,'advance' => $advance->balance, 'fix' => $advance->emi,'cnt_loan' => $loan->cnt, 'advance_loan' => $loan->balance, 'fix_loan' => $loan->emi, 'cnt_misc' => $misc->cnt,'advance_misc' => $misc->balance, 'fix_misc' => $misc->emi, 'status' => 1));
    }
    ...
    public function client_check($str) {
        ...
        $check = $this->cam->checkClient($str,$client_id,$branch_id,$month_year);
        if($check)
        {
            $this->form_validation->set_message('client_check', ' Client Name, Branch Name, Designation and Attendance Month should not be same ');
            return FALSE;
        }
        else
        {
