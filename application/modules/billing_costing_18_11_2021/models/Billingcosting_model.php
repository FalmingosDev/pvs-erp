...
class Billingcosting_model extends CI_Model {
	...
	public function getBillingCostingList($id){
		$query = $this->db->query("SELECT bch.billing_costing_cost_id,bch.client_id,bch.post_id,bch.gross_pay,bch.monthly_ctc,bch.annual_ctc,bch.total_bill_amt,bch.gross_profit,dm.desig_name,bch.status FROM billing_costing_header bch
		inner join designation_mst dm on dm.desig_id = bch.post_id where bch.client_id=".$id);
	...
	{
		$query = $this->db->query("Select DISTINCT desig_id,desig_name from designation_mst where active_status=1 ORDER BY desig_name ;");
		return $query->result();
	}
	...
	public function getMaxmin() {
      ...
      ,gross_profit,epf,esi,lwf,ptax,tot_dedn,net_pay,pay_hrs,ot_rate_type,ot_rt,esi_wage,ot_esi_wage,pf_wage FROM billing_costing_header where active_status=1 and client_id=".$client_id." and billing_costing_cost_id=".$billing_costing_id);
			return $qry->result();
		...
		public function edit_billing_costing_detail($client_id,$billing_costing_id){
			...
			$qry = $this->db->query("SELECT bcd.billing_costing_detail_id,bcd.billing_costing_header_id,bcd.salary_head_id,bcd.type,isnull(bcd.rate,'0') rate,bcd.basis,bcd.limit,bcd.criteria,bcd.periodicity,shm.max_prcntg,shm.min_prcntg FROM billing_costing_detail bcd inner join salary_head_mst shm on shm.head_id=bcd.salary_head_id where bcd.active_status=1 and bcd.billing_costing_header_id=".$billing_costing_id);
	...
	public function checkHeader($status,$desig_name,$commencement_date,$client_id,$branch_id) {
		$query = $this->db->query("SELECT count(*) cnt from [pvsdb].[dbo].[billing_costing_header] where commencement_date ='". $commencement_date ."' and post_id='". $desig_name ."' and status='". $status ."' and client_id='". $client_id."' and branch_id='". $branch_id."'");
		//echo $this->db->last_query();die;
		return $query->row();
	}
