<?php
class Delete_client_salary_model extends CI_Model {
	protected $tidTable = 'payroll_header';
	...
	public function get_all_month() {
		$query = $this->db->query("Select datename(m,payment_month)+' '+cast(datepart(yyyy,payment_month) as varchar) as month_year,payment_month FROM payroll_header ;");
		return $query->result();
	}
	...
	public function get_all_branch($month_year) {
		...
		INNER JOIN payroll_header pm ON pm.branch_id = bm.branch_id WHERE pm.payment_month ='". $month_year ."'
	...
	public function get_all_client($month_year) {
		...
		INNER JOIN payroll_header pm ON pm.client_id = cm.client_id WHERE pm.payment_month ='". $month_year ."'
	...
	$query = $this->db->query("EXEC delete_payroll_proc @p_payment_month = '".$payment_month."', @p_client_id = '".$client_id."', @p_branch_id='". $branch_id ."'");
