...
<section class="main-wrapper">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <?php echo form_open( base_url( 'delete_client_salary/delete' ), array( 'id' => 'loginform', 'class' => 'form-horizontal' ) );  ?>
                
                <div class="row">
                    <div class="col-md-7">
                        <h1 style="text-align: center;">Delete Client Salary Payment</h1>
                    </div>
                </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <?php 
                                        $attributes = array('class' => 'form-control form-control-sm', 'id' => 'month_year');
                        ...
                        <div class="col-md-3">
                            <div class="form-group row">
                                <label for="colFormLabelSm" class="col-sm-3 col-form-label col-form-label-sm">Client:    </label>
                                <div class="col-sm-9">
...
<script>
   $(document).ready(function() {
    ...
    $("#month_year").change(() => {
        ...
        populatebranch(month_year);
        populateclient(month_year);
        
    })


    function populatebranch(month_year){
    if (($("#month_year").val() != null)){
        ...
        $.ajax({
            type: 'POST',   
            url: '<?php echo base_url('delete_client_salary/branch_list'); ?>',
        ...
        .done(function(data){
            ...
            if(data.status){            
                ...
                $.each(data.branch_list,function(key,value){
                    result1 +='<option value="'+value.branch_id+'">'+value.branch_name+'</option>';
                });
            }
            else{
...
function populateclient(month_year){
    var result='';
    
    $.ajax({
        type: 'POST',   
        url: '<?php echo base_url('delete_client_salary/client_list'); ?>',
        data: {month_year: month_year,<?= $this->security->get_csrf_token_name(); ?>: $("[name='<?= $this->security->get_csrf_token_name(); ?>']").val()},
        dataType: 'json',
        encode: true,
    })
...
$.ajax({
 type:'POST',
 url: '<?php echo base_url('delete_client_salary/search'); ?>',
 data: {<?= $this->security->get_csrf_token_name(); ?>: $("[name='<?= $this->security->get_csrf_token_name(); ?>']").val(), month_year:month_year, client_id:client_id, branch_id:branch_id},
 dataType:'json',
 encode:true,
 ...
 success: function(data) {
    ...
    if(data.status){
        ...
        $.each(data.list,function(key,value){c++;
            ...
            html += '<td><i class="fa fa-trash" onclick="confirmation(\''+payment_month+ '\','+ value.client_id +','+ value.branch_id +');"></i></td>';                    
            html += '</tr>';
        })
    }
    else{
...
function confirmation(payment_month,client_id,branch_id){
        // alert(payment_month);
        var del=confirm("Are you sure you want to delete this record?\n"+ payment_month +'/'+ client_id +'/'+ branch_id);
        
        $.ajax({
			type: 'POST',   
			url: '<?php echo base_url('delete_client_salary/delete'); ?>',
			data: {payment_month:payment_month,client_id:client_id,branch_id:branch_id,<?= $this->security->get_csrf_token_name(); ?>: $("[name='<?= $this->security->get_csrf_token_name(); ?>']").val()},
			dataType: 'json',
			encode: true,
		})
		//ajax response
