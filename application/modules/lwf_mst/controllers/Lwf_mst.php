	...
	public function add()
	{
		if(!empty($this->input->post())){
			...
			if ($this->form_validation->run() == TRUE)
            {
				...
				if( $period == 'Y'){
					$date_year = $this->input->post('date_y');
				}
				else if( $period == 'H'){
					$date_year = $this->input->post('date_yy');
				}
				else {
					$date_year = NULL;
				}
				...
				//print_r($date_year);
				//print_r($date_half); exit;
				//$date_year = $this->input->post('date_yy');
				$date_half = $this->input->post('date_h');				
				...
				$query = $this->lm->insertlwf($state_id,$date_year,$date_half,$period,$employee_contribution,$employer_contribution,$total);
	...
	public function edit()
	{
		...
		if(!empty($this->input->post())){
			...
			if ($this->form_validation->run() == TRUE)
            {
				...
				$period_to = NULL;
				if($period == 'Y'){
					$period_from = $this->input->post('date_y');
				}
				else if($period == 'H'){
					$period_from = $this->input->post('date_y');
					$period_to = $this->input->post('date_h');
				}
				else
				{
					$period_from = NULL;
				...
				//print_r ($lwf_id);
				$query = $this->lm->editlwf($state_id,$period,$period_from,$period_to,$employee_contribution,$employer_contribution,$total,$lwf_id);
		...
		//print_r($data['lwfeditdetail']);die;
