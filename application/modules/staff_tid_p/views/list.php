...
<section class="main-wrapper">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <?php  echo form_open( base_url( 'staff_tid_p/staff_tid_p_pdf' ), array( 'id' => 'loginform', 'class' => 'form-horizontal' ) );  ?>
                <div class="row">
                    <div class="col-md-7">
                        <h1 style="text-align: center;">Staff Payment Bank Transfer Statement</h1>
                    </div>
                </div>
                    <div class="row">
                        <div class="col-md-3">
...
<script>
    ...
    function staff_search()
    {
		...
		$.ajax({
			type: 'POST',   
			url: '<?php echo base_url('staff_tid_p/search'); ?>',
            ...
            var pdf_result = '';
				...
				$.each(data.list,function(key,value){
					...
					if(value.tid != null){
                        ...
                        result +='<td>'+value.unit+'</td>';
						
					} 
					result +='</tr>';
					
				});
                var loc = "staff_tid_p/staff_tid_p_pdf?year="+year+ "& month=" +month+ "& tid=" +tid;
                // alert(loc);
                if($("#tid").val() != ''){
                    pdf_result += '<div class="add-btn-div">'; 
                    pdf_result += '<a href="<?php echo base_url('staff_tid_p/staff_tid_p_pdf'); ?>?year='+year+'&month='+month+'&tid='+tid+'" class="cr-a" >PDF Download</a> ';
                    pdf_result += '</div>';
