...
class Branch_model extends CI_Model {
  ...
  WHERE bm.client_id =".$client_id);
	...
	public function get_branch_edit($client_id,$branch_id) { 
		...
		$query = $this->db->query("SELECT bm.company_branch_id,bm.branch_id,bm.client_id,bm.branch_name,bm.address_1,bm.address_2,bm.address_3,bm.fax,
  ...
  WHERE bm.client_id =".$client_id. " AND bm.branch_id = ".$branch_id);
	...
	public function insertBranch($company_branch,$client_id,$address_line_1,$branch_name,$address_line_2,$fax,$address_line_3,$cont_no,$state_id,$client_email,$city_id,$sole_id,$pincode,$app_by,$name,$designation,$contact_no,$email,$birthday,$anniversary,$remarks,$branch_code,$branch_category_id)
	{
		$tot_name = count($name);
		$detail_data = '';
		...
		for($i=0; $i<$tot_name; $i++)
		{
			if(!empty($name[$i]) && !empty($designation[$i]) && !empty($contact_no[$i]))
				{
			$detail_data .= escape_str(strtoupper($name[$i])).'|~|'.escape_str(strtoupper($designation[$i])).'|~|'.$contact_no[$i].'|~|'.$email[$i].'|~|'.'2000-'.$birthday[$i].'|~|'.'2000-'.$anniversary[$i].'#';
				}
		} 
		...
		$query = $this->db->query("EXEC add_branch_proc @p_company_branch_id='".$company_branch."',@p_client_id='".$client_id."',@p_address_1='".escape_str(strtoupper($address_line_1))."',@p_address_2='".escape_str(strtoupper($address_line_2))."',@p_address_3='".escape_str(strtoupper($address_line_3))."',@p_branch='".escape_str(strtoupper($branch_name))."',@fax='".strtoupper($fax)."',@p_cont_no='".$cont_no."',@p_state_id='".$state_id."',@p_client_email='".$client_email."',@p_city_id='".$city_id."',@p_sole_id='".$sole_id."',@p_pincode='".$pincode."',@p_app_by='".escape_str(strtoupper($app_by))."',@p_remarks='".escape_str(strtoupper($remarks))."',@p_user_id = '".$user_id."', @p_detail_data = '". $detail_data."',@p_branch_code = '". $branch_code."',@p_branch_category = '". $branch_category_id."'");

		
		return $query->result();
	}
	
	public function updateBranch($company_branch,$client_id,$branch_id,$detail_id,$address_line_1,$branch_name,$address_line_2,$fax,$address_line_3,$cont_no,$state_id,$client_email,$city_id,$sole_id,$pincode,$app_by,$name,$designation,$contact_no,$email,$birthday,$anniversary,$remarks,$branch_category_id)
	{
		$tot_name = '';
		if(!empty($name)){
			$tot_name = count($name);
		...
		for($i=0; $i<$tot_name; $i++)
		{
			if(!empty($name[$i]) && !empty($designation[$i]) && !empty($contact_no[$i]))
				{
			$detail_data .= $detail_id[$i].'|~|'.escape_str(strtoupper($name[$i])).'|~|'.escape_str(strtoupper($designation[$i])).'|~|'.$contact_no[$i].'|~|'.$email[$i].'|~|'.'2000-'.$birthday[$i].'|~|'.'2000-'.$anniversary[$i].'#';
				}
		}
		//print_r($detail_data);die;
		 $detail_data = rtrim($detail_data, "#");
		...
		$query = $this->db->query("EXEC edit_branch_proc @p_company_branch='".$company_branch."',@p_client_id='".$client_id."',@p_branch_id='".$branch_id."',@p_address_1='".escape_str(strtoupper($address_line_1))."',@p_address_2='".escape_str(strtoupper($address_line_2))."',@p_address_3='".escape_str(strtoupper($address_line_3))."',@p_branch='".escape_str(strtoupper($branch_name))."',@fax='".strtoupper($fax)."',@p_cont_no='".$cont_no."',@p_state_id='".$state_id."',@p_client_email='".$client_email."',@p_city_id='".$city_id."',@p_sole_id='".$sole_id."',@p_pincode='".$pincode."',@p_app_by='".escape_str(strtoupper($app_by))."',@p_remarks='".escape_str(strtoupper($remarks))."',@p_user_id = '".$user_id."', @p_detail_data = '". $detail_data."',@p_branch_category = '". $branch_category_id."'");
	...
	{
		$query = $this->db->query("SELECT cm.company_branch_id,CONCAT(cm.branch_name,',',sm.state_name) as branch_name
	...
	public function getBranchCode() {
    	//echo "1";die;
		$query = $this->db->query("SELECT [dbo].[getBranchCode] () as branch_code");
		return $query->row();
	}
