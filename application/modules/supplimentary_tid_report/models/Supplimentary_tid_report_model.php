<?php
class Supplimentary_tid_report_model extends CI_Model {
	...
	public function get_all_records($payment_year,$payment_month) {
		...
		$condition .= " AND pm.payroll_type ='CS'";
		$query = $this->db->query("SELECT distinct(pm.tid) tid,fm.client_code,fm.client_name,bm.branch_name,isnull(bm.branch_code,'') as branch_code FROM lock_salary pm INNER JOIN client_mst fm ON fm.client_id = pm.client_id INNER JOIN branch_mst bm ON bm.branch_id = pm.branch_id " . $condition . " ;");
