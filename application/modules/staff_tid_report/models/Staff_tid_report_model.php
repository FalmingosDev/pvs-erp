<?php
class Staff_tid_report_model extends CI_Model {
	...
	public function get_all_records($payment_year,$payment_month) 
	{
		...
		$condition .= " AND pm.payroll_type ='SP'";
		
		$query = $this->db->query("SELECT distinct(pm.tid) tid,isnull(fm.employee_code,'')as client_code ,concat(fm.first_name,' ',fm.last_name) AS client_name,bm.branch_name FROM lock_salary pm 
		INNER JOIN employee_mst fm ON fm.employee_id = pm.client_id 
