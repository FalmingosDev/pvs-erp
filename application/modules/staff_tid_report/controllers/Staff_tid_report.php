...
class Staff_tid_report extends MY_Controller 
{
	...
	public function __construct() 
	{
		...
		$this->load->model('Staff_tid_report_model','stm');
	...
	public function index()
	{
		...
		//$data['list'] = $this->stm->get_all_records($data['payment_year'], $data['payment_month']);
		
		$month_obj = $this->stm->get_all_month();
		...
		$year_obj = $this->stm->get_all_year();
	...
	public function client_tid_list()
	{
		...
		// $tid=$this->input->post('tid');
		$data = array();
		$list = $this->stm->client_tid_list($account_no,$reference_no,$net_pay,$bank_name,$employee_code,$payment_month,$payment_year,$tid,$employee_name,$employee_desig,$payment_date,$client_code,$client_name,$branch_name);
		//print_r($list);exit();
	...
	public function search()
	{
		...
		$data['list'] = $this->stm->get_all_records($year,$month);
	...
	{	 
		...
		//print_r ($month); exit;
		 ...
		 $sheet->setCellValue('D2', 'Branch Name');
		...
		$list = $data['list'] = $this->stm->get_all_records($year,$month);
		//print_r($data);exit;
