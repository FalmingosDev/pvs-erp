...
class Billing_report extends MY_Controller 
{
	...
	public function generate_excel()
	{
		if (!empty($this->input->post('month')) && !empty($this->input->post('client_id')))
		{
			...
			$sheet->setCellValue('D1', 'Address of the Branch');
			$sheet->setCellValue('E1', 'Branch Name');
			$sheet->setCellValue('F1', 'Branch ID as per Bank');
			$sheet->setCellValue('G1', 'Branch Category as per Bank');
			$sheet->setCellValue('H1', 'Category of City as per labour notification');
			$sheet->setCellValue('I1', 'Applicability of Wages as per (Central / State)');
			$sheet->setCellValue('J1', 'Contractual Manpower Cost (Wages Per Day)');
			$sheet->setCellValue('K1', 'No. of Staff');
			$sheet->setCellValue('L1', 'No. of Days present');
			$sheet->setCellValue('M1', 'Wages to be paid ');
			$sheet->setCellValue('N1', 'EPF @ 13%');
			$sheet->setCellValue('O1', 'ESIC on Basic wages  @ 3.15%');
			$sheet->setCellValue('P1', 'Uniform');
			$sheet->setCellValue('Q1', 'Bonus @ 8.33% on MW');
			$sheet->setCellValue('R1', 'Sub Total');
			$sheet->setCellValue('S1', 'Mangement Fee');  
			$sheet->setCellValue('T1', 'Gross Amount');
			$sheet->setCellValue('U1', 'SGST');
			$sheet->setCellValue('V1', 'CGST');
			$sheet->setCellValue('W1', 'IGST');
			$sheet->setCellValue('X1', 'UGST');
			$sheet->setCellValue('Y1', 'Total Billing Amount');
			$styleArray = array(
				'font'  => array(
				'bold'  => true,
			...
			foreach($list as $list_row)
			{
				...
				$sheet->setCellValue('B'.$StCount, $list_row->client_name);
				$sheet->setCellValue('C'.$StCount, $list_row->state_name);
				$sheet->setCellValue('D'.$StCount, $list_row->address);
				$sheet->setCellValue('E'.$StCount, $list_row->branch_name);
				$sheet->setCellValue('F'.$StCount, $list_row->branch_code);
				$sheet->setCellValue('G'.$StCount, $list_row->branch_category);
				$sheet->setCellValue('H'.$StCount, '');
				$sheet->setCellValue('I'.$StCount, $list_row->gross_pay);
				$sheet->setCellValue('J'.$StCount, $list_row->gross_pay_per_day);
				$sheet->setCellValue('K'.$StCount, $list_row->nu_of_staff);
				$sheet->setCellValue('L'.$StCount, $list_row->payment_days);
				$sheet->setCellValue('M'.$StCount, $list_row->total_gross_pay);
				$sheet->setCellValue('N'.$StCount, $list_row->epf);
				$sheet->setCellValue('O'.$StCount, $list_row->esi);
				$sheet->setCellValue('P'.$StCount, $list_row->uniform_deduct);
				$sheet->setCellValue('Q'.$StCount, $list_row->bonus);
				$sheet->setCellValue('R'.$StCount, $list_row->sub_total);
				$sheet->setCellValue('S'.$StCount, $list_row->services_free);
				$sheet->setCellValue('T'.$StCount, $list_row->gross_amount);
				$sheet->setCellValue('U'.$StCount, $list_row->sgst);
				$sheet->setCellValue('V'.$StCount, $list_row->cgst);
				$sheet->setCellValue('W'.$StCount, $list_row->igst);
				$sheet->setCellValue('X'.$StCount, $list_row->ugst);
				$sheet->setCellValue('Y'.$StCount, $list_row->total_billing_ammount);
				$StCount++;
				$sn++;
			} 
			...
			$sheet->setCellValue('L'.$StCount, '');
			$sheet->setCellValue('M'.$StCount, $total_gross_pay);
			$sheet->setCellValue('N'.$StCount, $total_epf);
			$sheet->setCellValue('O'.$StCount, $total_esi);
			$sheet->setCellValue('P'.$StCount, $total_uniform_deduct);
			$sheet->setCellValue('Q'.$StCount, $total_bonus);
			$sheet->setCellValue('R'.$StCount, $total_subtotal);
			$sheet->setCellValue('S'.$StCount, $total_services_free);
			$sheet->setCellValue('T'.$StCount, $total_gross_amount);
			$sheet->setCellValue('U'.$StCount, $total_sgst);
			$sheet->setCellValue('V'.$StCount, $total_cgst);
			$sheet->setCellValue('W'.$StCount, $total_igst);
			$sheet->setCellValue('X'.$StCount, $total_ugst);
			$sheet->setCellValue('Y'.$StCount, $total_billing_ammount);
