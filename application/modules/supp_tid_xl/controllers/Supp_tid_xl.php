...
class Supp_tid_xl extends MY_Controller 
{
	...
	public function __construct() 
	{
		...
		$this->load->model('Supp_tid_xl_model');
	...
	public function index()
	{
		...
		if(check_uri_permission('supp_tid_xl', 'V') == 0){
		...
		$data['tid']  = $this->input->get('tid');
		 ...
		 $data['list'] = $this->Supp_tid_xl_model->get_all_records($data['payment_year'], $data['payment_month'],$data['tid']);
		// $data['list'] = $this->Supp_tid_xl_model->get_all_records($data['payment_month'],$data['tid']);
		
		$month_obj = $this->Supp_tid_xl_model->get_all_month();
		...
		$year_obj = $this->Supp_tid_xl_model->get_all_year();
		...
		$tid_obj = $this->Supp_tid_xl_model->get_all_tid();
		//print_r($tid_obj);die;
		...
		$this->template->write('title', 'Supplimentary Payment TID List', TRUE);
	...
	public function client_tid_list()
	{
		...
		$data = array();
		$list = $this->Supp_tid_xl_model->client_tid_list($account_no,$reference_no,$net_pay,$bank_name,$employee_code,$payment_month,$payment_year,$tid,$employee_name,$employee_desig,$payment_date,$client_code,$client_name,$branch_name);
		//print_r($list);exit();
	...
	public function search_list()
	{
		...
		$data['list'] = $this->Supp_tid_xl_model->get_all_records($year,$month,$tid);
	...
	public function supp_tid_xl_excel()
	{	 
		...
		$list = $data['list'] = $this->Supp_tid_xl_model->get_all_records($year,$month,$tid);
		//print_r($data);exit;
