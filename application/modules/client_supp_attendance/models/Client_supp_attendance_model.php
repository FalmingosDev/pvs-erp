...
class Client_supp_attendance_model extends CI_Model {
  ...
  public function getDesignation($client_id) {
    $query = $this->db->query("Select d.desig_id,d.desig_name from designation_mst d inner join billing_costing_header bh on d.desig_id = bh.post_id where d.active_status=1 and bh.client_id='". $client_id ."' ORDER BY d.desig_name;");
  ...
  public function getBillingCosting($client_id,$designation_id) {
    $query = $this->db->query("SELECT top 1 salary_type,billing_type,salary_days,bill_calculation_days,duty_hrs,commencement_date from billing_costing_header where client_id=". $client_id ." and post_id=" . $designation_id . " and status='Active' and GETDATE() > commencement_date order by commencement_date desc");
    return $query->row();
  }

  public function insertClientAttendance($attendance_month,$client_id,$branch_id,$employee_id,$flag,$designation_id,$calculation_days,$duty_hrs,$duty,$w_o,$leave,$el,$fl,$c_o,$ph,$ot,$na_duty,$na) {
  	//echo "1";die;
    //echo $half_day;die;
    $user_id = $this->session->user_id;
    $final_attendance_month = date("Y-m-d", strtotime($attendance_month));
    
    $query = $this->db->query("EXEC add_client_supp_attendance_proc @p_client_id = '".$client_id."', @p_branch_id='". $branch_id ."', @p_employee_id = '".$employee_id."' , @p_flag = '".$flag."', @p_designation_id = '".$designation_id."', @p_attendance_month = '".$final_attendance_month."' ,@p_calculation_days = '".$calculation_days."' , @p_duty_hrs = '".floatval($duty_hrs)."', @p_duty = '".floatval($duty)."', @p_wo = '".floatval($w_o)."', @p_leave = '".floatval($leave)."', @p_el = '".floatval($el)."', @p_fl = '".floatval($fl)."', @p_co = '".floatval($c_o)."', @p_ph = '".floatval($ph)."', @p_ot = '".floatval($ot)."', @p_na_duty = '".floatval($na_duty)."', @p_na = '".floatval($na)."', @p_user_id = '".$user_id."'");
    //echo $this->db->last_query();die;
  ...
  public function getClientName() {
    $query = $this->db->query("Select cm.client_name,cm.client_id from client_supp_attendance ca inner join client_mst cm on cm.client_id = ca.client_id where ca.active_status=1 ORDER BY cm.client_name;");
    return $query->result();
  }

  public function getMonth() {
    $query = $this->db->query("Select datename(m,attendance_month)+' '+cast(datepart(yyyy,attendance_month) as varchar) as MonthYear,attendance_month from client_supp_attendance where active_status=1;");
    return $query->result();
  }
  ...
  public function getClientDetails($id){
	...
	public function updateAttendanceApproval($client_attendance_id, $approval_remarks, $action) {
		$query = $this->db->query("EXEC update_client_supp_attendance_approval_proc @p_client_supp_attendance_id = '". $client_attendance_id ."', @p_approval_remarks = '".$approval_remarks."', @p_action = '".$action."', @p_user_id = '". $this->session->user_id ."';");
