...
class Client_supp_attendance extends MY_Controller {
	...
	public function __construct() {
        ...
        $this->load->model('Client_supp_attendance_model','cam');
	}
	...
	public function add_client_attendance(){
        ...
        if(check_uri_permission('client_supp_attendance/client', 'A') == 0){
         $this->session->set_flashdata('error_msg', 'You have no permission on this page');
         redirect(base_url());
         }
              ...
              if ($this->form_validation->run() == TRUE)
              {
                   ...
                   // $other_deduct = $this->input->post('other_deduct');
                    ...
                    $na = $this->input->post('na');
             		...
             		$qry = $this->cam->insertClientAttendance($attendance_month,$client_id,$branch_id,$employee_id,$flag,$designation_id,$calculation_days,$duty_hrs,$duty,$w_o,$leave,$el,$fl,$c_o,$ph,$ot,$na_duty,$na);

            //  //    $empd_id = $qry['empd_id'];
                if($qry)
                {
                    $this->session->set_flashdata('msg', 'Data Saved Successfully');
                    redirect(base_url().'client_supp_attendance/add_client_attendance');
                }
    ...
    public function edit_client_attendance($client_attendance_id)
    {
        ...
        if(check_uri_permission('client_supp_attendance/client', 'E') == 0){
         $this->session->set_flashdata('error_msg', 'You have no permission on this page');
         redirect(base_url());
         }
      ...
      if(!empty($this->input->post())){
            ...
            if ($this->form_validation->run() == TRUE){
                    $client_attendance_id = $this->input->post('client_supp_attendance_id');
                    $duty = $this->input->post('duty');
                    $w_o = $this->input->post('w_o');
                    $leave = $this->input->post('leave');
                    $el = $this->input->post('el');
                    ...
                    $qry = $this->cam->UpdateClientAttendance($client_attendance_id,$duty,$w_o,$leave,$el,$fl,$c_o,$ph,$ot,$na_duty,$na);
                    if($qry)
                    {
                        $this->session->set_flashdata('msg', 'Data Saved Successfully');
                        redirect(base_url().'client_supp_attendance/edit_client_attendance/'.$client_attendance_id);
                    }
              }
	...
	public function approve($client_attendance_id){
		...
		if(!empty($this->input->post())){
			...
			if ($this->form_validation->run() == TRUE){
				...
				if($qry) {
					$this->session->set_flashdata('msg', 'Data Saved Successfully');
					redirect(base_url('client_supp_attendance/client'));
				}
