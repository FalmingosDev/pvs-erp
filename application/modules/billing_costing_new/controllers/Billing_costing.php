...
class Billing_costing extends MY_Controller {
            ...
            if ($this->form_validation->run() == TRUE)
             {
                ...
                $basic_highly_skilled = $this->input->post('basic_highly_skilled');
                $basic_skilled = $this->input->post('basic_skilled');
                $basic_semi_skilled = $this->input->post('basic_semi_skilled');
                $basic_un_skilled = $this->input->post('basic_un_skilled');

                $da_highly_skilled = $this->input->post('da_highly_skilled');
                $da_skilled = $this->input->post('da_skilled');
                $da_semi_skilled = $this->input->post('da_semi_skilled');
                $da_un_skilled = $this->input->post('da_un_skilled');

                $overtime_highly_skilled = $this->input->post('overtime_highly_skilled');
                $overtime_skilled = $this->input->post('overtime_skilled');
                $overtime_semi_skilled = $this->input->post('overtime_semi_skilled');
                $overtime_un_skilled = $this->input->post('overtime_un_skilled');

                $effective_date = $this->input->post('effective_date');

                if(empty($basic_highly_skilled))
                    {
                        $basic_highly_skilled=0;
                    }
                    if(empty($basic_skilled))
                    {
                        $basic_skilled=0;
                    }
                    if(empty($basic_semi_skilled))
                    {
                        $basic_semi_skilled=0;
                    }
                    if(empty($basic_un_skilled))
                    {
                        $basic_un_skilled=0;
                    }
                    if(empty($da_highly_skilled))
                    {
                        $da_highly_skilled=0;
                    }
                    if(empty($da_skilled))
                    ...
                    if(empty($da_semi_skilled))
                    {
                        $da_semi_skilled=0;
                    }
                    if(empty($da_un_skilled))
                    {
                        $da_un_skilled=0;
                    }
                    if(empty($overtime_highly_skilled))
                    {
                        $overtime_highly_skilled=0;
                    }
                    if(empty($overtime_skilled))
                    {
                        $overtime_skilled=0;
                    }
                    if(empty($overtime_semi_skilled))
                    {
                        $overtime_semi_skilled=0;
                    }
                    if(empty($overtime_un_skilled))
                    {
                        $overtime_un_skilled=0;
                    }
    ...
    //echo "1";die;
		...
		echo json_encode(array('salaryhead_list' => $data['salaryhead_list'],'status' => 1));
	}
    ...
    public function edit()
    {
        ...
        $data['edit_billing_costing_skilled_detail'] = $this->bcm->edit_billing_costing_skilled($billing_costing_id);
        $data['pf_percentage'] = $this->bcm->get_pf_percentage();
        $data['esi_percentage'] = $this->bcm->get_esi_percentage();
        ...
        if(!empty($this->input->post()))
        {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                    ...
                    $effective_date = $this->input->post('effective_date');
                    $billing_costing_skilled_id = $this->input->post('billing_costing_skilled_id');
                   ...
                   /* $skilled_data = $basic_highly_skilled.'|~|'.$basic_skilled.'|~|'.$basic_semi_skilled.'|~|'.$basic_un_skilled.'|~|'.
                                    $da_highly_skilled.'|~|'.$da_skilled.'|~|'.$da_semi_skilled.'|~|'.$da_un_skilled.'|~|'.
                                    $overtime_highly_skilled.'|~|'.$overtime_skilled.'|~|'.$overtime_semi_skilled.'|~|'.$overtime_un_skilled.'|~|'.$effective_date.'|~|'.$billing_costing_skilled_id;*/
